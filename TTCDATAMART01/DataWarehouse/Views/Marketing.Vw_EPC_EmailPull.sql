...
SELECT epc.*,med.MaxOpenDate    
  ...
  left join (select * from [Staging].[vw_EU_Countries] Except Select *  from Mapping.TGCPlusCountry where Alpha2Code in ('GB') ) GDPR  
  on epc.CountryCode = GDPR.Alpha2Code  /* GDPR Country filters*/  
  where GDPR.Alpha2Code is null  
