...
CREATE View [Marketing].[Vw_EPC_Prospect_EmailPull_OSW]  
as  

SELECT * FROM Marketing.EPC_Prospect_EmailPull EPC
WHERE EPC.Emailaddress IN /* Only include after 35 days end of Prospect wave VB 3/7/2017 */
(SELECT RTRIM(LTRIM(email)) 
FROM MagentoImports..epc_preference p 
WHERE p.registration_source_id = 7 AND DATEDIFF(d,p.created_date,GETDATE())<35 ) 
...
lusiveOffers,
	...
	case when P.Store_id  = 1 then 'US'
		 ...
		 else ECI.store_country end as store
_country,
	case when P.Store_id  = 1 then 'US'
...
left join MagentoImports..Email_Customer
_Information ECI 
...
and EPC.Snoozed = 0 
and EPC.hardbounce = 0 and EPC.Softbounce = 0 and EPC.Black
list = 0  
and isnull(EPC.CustomerID ,'')=''
and EPC.Email like '%@%'
/* Only include after 35 days end of Prospect wave VB 3/7/2017 */
AND epc.Email NOT IN (SELECT RTRIM(LTRIM(email)) FROM MagentoImports..epc_preference p WHERE p.registration_source_id =
 7 AND DATEDIFF(d,p.created_date,GETDATE())>35 ) 
