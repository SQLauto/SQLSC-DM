...
/*select a.CustomerID, 
	...
	c.MD_Audience,
	c.MD_PriceType,
	Case when a.StockItemID like 'PD%' then 'DVD'
		 when a.StockItemID like 'PC%' then 'CD'
		 when a.StockItemID like 'PT%' then 'Physical Transcript'
		 when a.StockItemID like 'DV%' then 'DownloadV '
		 when a.StockItemID like 'LD%' then 'Library DVD'
		 when a.StockItemID like 'DA%' then 'DownloadA'
		 else 'Others' end 	 as Format,
	count(a.OrderID) Orders,					
	sum(a.TotalSales) Sales,
	GETDATE() as ReportDate					
from DataWarehouse.Marketing.DMPurchaseOrderitems a with (nolock) join						
	(select *					
	from DataWarehouse.Marketing.CampaignCustomerSignature with (nolock)
	where CountryCode = 'US'					
	and PublicLibrary = 1)b on a.CustomerID = b.CustomerID	left join
