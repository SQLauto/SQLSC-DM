...
CREATE Proc [dbo].[SP_Load_TGC_TGCplus_1] @StartDate date = null,@EndDate Date = null
as
Begin 
...
/* Email Matches between the @StartDate and @EndDate dates #TGCPlusEmail */    /* Note we may have Multiple emails for Same CustomerID in EPC*/
		...
		on a.email = d.Email
		Left join Datawarehouse.Mapping.TGC_TGCplus_1 Map
...
-- table #EmailMatch
		...
		--select * from #EmailMatchFinal
		 ...
		 Update #EmailMatchFinal
		 set NewSeg = case when NetOrderAmount<=101 then 3 when NetOrderAmount between 101 and 220 then 4 else 5 end,
			 Name =  case when NetOrderAmount<=101 then '6m1' when NetOrderAmount between 101 and 220 then '6m2' else '6m3' end,
			 a12mf = 2,
			 ComboID = case when NetOrderAmount<=101 then '36m12' when NetOrderAmount between 101 and 220 then '46m22' else '56m32' end,
			 CustomerSegment = 'Active',
			 Frequency ='F2',
			 CustomerSegmentNew = 'Active',
			 CustomerSegmentFnl = 'Active_Multi',
			 CustomerSegmentFrcst = 'Active_Multi'   
		--select * from #EmailMatchFinal
		 where FirstCustBUSub = 'TGC' and TGCPurchaseFlag = 1
	...
	Insert into Datawarehouse.Mapping.TGC_TGCplus_1
	...
	from #TGC_Customer c
	left join (select Customerid,TGCP_ID from Datawarehouse.Mapping.TGC_TGCplus_1) Map
	...
	where Map.Customerid is null  and  Map.TGCP_ID is null
		...
		--select E.* 
		from Datawarehouse.Mapping.TGC_TGCplus_1 Map
		...
		select Count(*) Cnt 
		from Datawarehouse.Mapping.TGC_TGCplus_1 
