...
CREATE Proc [Staging].[SP_VL_Load_VideoEvents_20160325]   
as  
Begin  
...
select cast(tstamp as date)Deleting_data_for_these_dates from Datawarehouse.staging.VL_ssis_VideoEvents group by cast(tstamp as date)
Print 'Deleted'
delete from  Datawarehouse.archive.TGCPlus_VideoEvents  
--where tstamp >= (select cast(isnull(MIN(tstamp),getdate()) as date) from Datawarehouse.staging.VL_ssis_VideoEvents )  
