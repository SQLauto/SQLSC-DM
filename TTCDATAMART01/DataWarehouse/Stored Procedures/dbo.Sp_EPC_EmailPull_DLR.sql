...
CREATE NONCLUSTERED INDEX [IX_EPC_EmailPull_Customerid]      
ON [Staging].[EPC_EmailPull] ([CustomerID])                                           
...
if OBJECT_ID('staging.CANSPAM') is not null      
...
--------------------------------------------------------------------------------------------------------------------------------------------                                  
--------------------------------------------------------------------------------------------------------------------------------------------                                
-- If priority is for plus, then drop all plus customers.      
-- PR 5/10/2018      
...
if @Priority like '%Plus%'       
 begin      
  delete a      
  from staging.EPC_EmailPull a join      
   mapping.TGC_TGCplus b on a.customerid = b.CustomerID      
      
  delete a      
  from staging.EPC_EmailPull a join      
   mapping.TGC_TGCplus b on a.Emailaddress = b.TGCP_EmailAddress      
      
  delete a      
  from staging.EPC_EmailPull a join      
   Marketing.TGCPlus_CustomerSignature b on a.customerid = b.TGCCustomerID      
      
  delete a      
  from staging.EPC_EmailPull a join      
   Marketing.TGCPlus_CustomerSignature b on a.Emailaddress = b.EmailAddress      
