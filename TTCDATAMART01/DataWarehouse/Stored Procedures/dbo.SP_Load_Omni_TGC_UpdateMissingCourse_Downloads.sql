...
SET ANSI_NULLS ON
GO
CREATE Proc [dbo].[SP_Load_Omni_TGC_UpdateMissingCourse_Downloads]
as
Begin
select distinct MediaName 
into #web_Downloads
from DataWarehouse.Archive.Omni_TGC_web_Downloads
...
on left(a.MediaName,16) = left(l.akamai_download_id,16)
 ...
 from DataWarehouse.Archive.Omni_TGC_web_Downloads S
 join #MediaName m 
...
select distinct MediaName 
into #APPS_Downloads
from DataWarehouse.Archive.Omni_TGC_APPS_Downloads
where courseid is null
...
on a.MediaName = l.audio_brightcove_id
 ...
 S.LectureNumber = m.LectureNumber ,
 S.lecture_duration = m.lecture_duration 
 from DataWarehouse.Archive.Omni_TGC_Apps_Downloads S
