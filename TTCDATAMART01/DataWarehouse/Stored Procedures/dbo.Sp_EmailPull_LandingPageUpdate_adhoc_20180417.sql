...
CREATE Proc [dbo].[Sp_EmailPull_LandingPageUpdate_adhoc_20180417]  @emailid varchar(200)      
as      
Begin      
...
Select  @startdate = cast(dateadd(d,0,max(startdate)) as DATE),      
...
select @PrefID = convert(varchar,Prefid)      
from DataWarehouse.Mapping.WebLP_PrefID      
...
exec [Staging].[CreateWebLandingPageData] 'Email', 'RankBySales', 12, @countrycd, @emailid, @startdate, @stopdate,@PrefID      
--exec [Staging].[CreateWebLandingPageData] 'Email', 'PreRank', null, 'AU', 'PreRankTEST', '9/10/2014', '9/21/2014',65      
...
-- Float %off Messaging Courses on top.  --11.33
...
set @sql = 'update a       
   ...
   inner join DataWarehouse.Mapping.email_landingpage LP      
   on LP.CourseID = a.CourseID      
   where LP.EmailID = ''' + @emailid   + '''   and LP.Message like ''%[%]%'' '       
