...
SET ANSI_NULLS ON
GO
CREATE PROC [Staging].[CampaignEmail_FvrtSubjNew_R3PFG]
...
begin
    ...
    IF @DeBugCode = 1 PRINT 'Generate Ranks Table'
        ...
        if object_id('Staging.Ecampn02Rank_R3PFG') is not null drop table Staging.Ecampn02Rank_R3PFG

    SELECT Sum(FNL.SumSales) AS SumSales,
        FNL.CourseID,
        FNL.PreferredCategory,
        'R3PFG' as PrefVariable,
		CONVERT(FLOAT,0) AS Rank
    INTO Staging.Ecampn02Rank_R3PFG
    FROM	(SELECT sum(OI.SalesPrice) AS SumSales,
                II.Courseid AS CourseID,
                'XX-X-XX-XX' AS PreferredCategory
        ...
        SELECT sum(OI.SalesPrice) AS SumSales,
            II.Courseid AS CourseID,
            ISNULL(CSM.CRComboID,'XX-X-XX-XX') AS PreferredCategory
        FROM 
...
--        AND CSM.PreferredCategory2 is not null
        GROUP BY CSM.CRComboID, II.Courseid)FNL
    ...
    WHILE @@FETCH_STATUS = 0
        BEGIN
            ...
            /*- Assign rank based on sales for the course id within the PreferredCategory*/
                UPDATE Staging.Ecampn02Rank_R3PFG
                SET Rank = @Rank
                WHERE PreferredCategory = @PrefCat
 ...
 --NB
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    INSERT INTO Staging.Ecampn02Rank_R3PFG
                    SELECT @SumSales3,@CourseID3,@PrefCat,@Rank
...
--UPDATE Staging.Ecampn02Rank_R3PFG
--SET Rank = rank/8.2
--WHERE CourseID IN (3356,1495,7158,2353,2598,888,6537,1295,4617,8100,710,625,4244,1720)



    /*- Report1: Ranking check report*/
    IF EXISTS (SELECT 8 FROM sysobjects WHERE Name = 'Ecampn02Rank_R3PFGBKP')
        DROP TABLE Staging.Ecampn02Rank_R3PFGBKP
    	
    select *
    into Staging.Ecampn02Rank_R3PFGBKP
    from Staging.Ecampn02Rank_R3PFG


    SELECT a.Sumsales,a.CourseID,b.CourseName,a.PreferredCategory, a.Rank   
    FROM Staging.Ecampn02Rank_R3PFG a,
        Mapping.DMCourse b
    WHERE a.CourseID = b.CourseID
    ORDER BY a.PreferredCategory, a.Rank

    /* delete courses ranked higher than 100 to save time*/
    delete from Staging.Ecampn02Rank_R3PFG
    where rank > 80
    ...
    IF EXISTS (SELECT 8 FROM sysobjects WHERE Name = 'Ecampn03Customer01_R3PFG')
        DROP TABLE Staging.Ecampn03Customer01_R3PFG
    ...
    SELECT ccs.Customerid,
        ...
        END as PreferredCategory2,
        ccs.CRComboID as PreferredCategory,
        'R3PFG' as PrefVariable,
    ...
    INTO Staging.Ecampn03Customer01_R3PFG
    ...
    IF @FlagIntnl = 'YES' 
       BEGIN
        INSERT INTO Staging.Ecampn03Customer01_R3PFG
        (Customerid, FirstName, LastName, 
        EmailAddress, ComboID, Adcode, 
        Region, CountryCode, SubjectLine, 
        PreferredCategory2, PreferredCategory, PrefVariable,
        ECampaignID, CustomerSegmentNew)
        SELECT CCS.CustomerID, Staging.proper(CCS.FirstName) FirstName,
            ...
            'GEN' As  PreferredCategory2,
            'XX-X-XX-XX' AS PreferredCategory,
            'R3PFG' as PrefVariable,
            @ECampaignIDInq AS ECampaignID,
            ccs.CustomerSegmentNew
        FROM Marketing.CampaignCustomerSignature CCS LEFT OUTER JOIN
            Staging.Ecampn03Customer01_R3PFG EC01 ON CCS.CustomerID = EC01.CustomerID
    ...
    DELETE FROM Staging.Ecampn03Customer01_R3PFG
    WHERE CountryCode = 'GB'



    DELETE A
    from Staging.Ecampn03Customer01_R3PFG a join
    ...
    FROM Staging.Ecampn03Customer01_R3PFG EC01 
    ...
    /* 		Staging.Ecampn03Customer01_R3PFG EC01 on WI.Customerid = EC01.CustomerID*/
    ...
    WHERE CustomerID IN (
            ...
            FROM Staging.Ecampn03Customer01_R3PFG 
    ...
    exec Staging.CampaignEmail_RemoveUnsubs 'Ecampn03Customer01_R3PFG'
    ...
    /*delete a*/
    /*from Ecampn03Customer01_R3PFG a left outer join*/
    ...
    */
    ...
    IF EXISTS (SELECT 8 FROM sysobjects WHERE Name = 'Ecampn03Customer02_Course_R3PFG')
        DROP TABLE Staging.Ecampn03Customer02_Course_R3PFG
    ...
    SELECT a.Customerid,
        ...
        a.SubjectLine,
        a.PreferredCategory2,
        a.PreferredCategory,
        a.PrefVariable,
        a.EcampaignID,
        a.CustomerSegmentNew 
    INTO Staging.Ecampn03Customer02_Course_R3PFG
    FROM Staging.Ecampn03Customer01_R3PFG a,
        Staging.Ecampn02Rank_R3PFG er
    WHERE ISNULL(a.PreferredCategory,'XX_X_XX_XX') = er.PreferredCategory


    CREATE CLUSTERED INDEX IDX_customer_Ecamp03Cust02
    ON Staging.Ecampn03Customer02_Course_R3PFG (Customerid)
    ...
    DELETE a
    /*SELECT a.* */
    FROM Staging.Ecampn03Customer02_Course_R3PFG a,
        ...
        AND courseid IN (SELECT DISTINCT courseid FROM Staging.Ecampn02Rank_R3PFG))b
    ...
    IF EXISTS (SELECT 8 FROM sysobjects WHERE Name = 'Ecampn04Customer_Final_R3PFG')
        DROP TABLE Staging.Ecampn04Customer_Final_R3PFG

    CREATE TABLE Staging.Ecampn04Customer_Final_R3PFG
    ...
    ON Staging.Ecampn04Customer_Final_R3PFG (Customerid)


    IF @DeBugCode = 1 PRINT 'STEP 6: Rerank the course list to put them in order'

    IF EXISTS (SELECT 8 FROM sysobjects WHERE Name = 'Ecampn03Customer02_Course_R3PFG2')
        DROP TABLE Staging.Ecampn03Customer02_Course_R3PFG2

    select *, rank() over (partition by customerid order by rank) as Rank2
    into Staging.Ecampn03Customer02_Course_R3PFG2
    from Staging.Ecampn03Customer02_Course_R3PFG


    CREATE INDEX ix_Ecampn03Customer02_Course_R3PFG21 on Staging.Ecampn03Customer02_Course_R3PFG2 (Customerid)
    CREATE INDEX ix_Ecampn03Customer02_Course_R3PFG22 on Staging.Ecampn03Customer02_Course_R3PFG2 (rank2)


    INSERT INTO Staging.Ecampn04Customer_Final_R3PFG
    ...
    FROM Staging.Ecampn03Customer02_Course_R3PFG2
    WHERE rank2 <= 25
    ...
    */
    ...
    /*
    IF EXISTS (SELECT 8 FROM sysobjects WHERE Name = 'Ecampn04Customer_Final_R3PFG_New')
        DROP TABLE Staging.Ecampn04Customer_Final_R3PFG_New

    SELECT ECC.*, ECCSeq.SeqNum
    INTO Staging.Ecampn04Customer_Final_R3PFG_New
    FROM Staging.Ecampn03Customer02_Course_R3PFG ECC JOIN
                (SELECT ECC1.CUSTOMERID, ECC1.RANK, 
                SUM(CASE WHEN ECC2.RANK <= ECC1.RANK THEN 1 ELSE 0 END) SEQNUM 
                 FROM Staging.Ecampn03Customer02_Course_R3PFG ECC1 JOIN
                     Staging.Ecampn03Customer02_Course_R3PFG ECC2
    ...
    ON Staging.Ecampn04Customer_Final_R3PFGNew (Customerid)
    */
    ...
    DELETE FROM Staging.Ecampn04Customer_Final_R3PFG
    WHERE CustomerID IN 
            (SELECT CustomerID
            FROM Staging.Ecampn04Customer_Final_R3PFG
            GROUP BY CustomerID
            HAVING COUNT(*) < 5)

    /* DELETE FROM Staging.Ecampn04Customer_Final_R3PFG_New*/
    /* WHERE CustomerID IN */
    /* 		(SELECT CustomerID*/
    /* 		FROM Staging.Ecampn04Customer_Final_R3PFG_New*/
    /* 		GROUP BY CustomerID*/
    /* 		HAVING COUNT(*) < 5)*/
    ...
    /* FROM Staging.Ecampn04Customer_Final_R3PFG mct,*/
    ...
    /* FROM Staging.Ecampn04Customer_Final_R3PFG fnl LEFT OUTER JOIN*/
    ...
    /* 	Staging.Ecampn04Customer_Final_R3PFG d*/
    ...
    FROM (SELECT MAX(DISTINCT a.customerid) AS customerid,
        ...
        FROM Staging.Ecampn04Customer_Final_R3PFG  a,
            Staging.Ecampn03Customer01_R3PFG b
        WHERE a.customerid=b.customerid
