...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [Staging].[GetDailyTopLineReport] 
AS
	-- Preethi Ramanujam    2/6/2012 - Get daily top line report
begin

    IF EXISTS (SELECT 8 FROM sysobjects WHERE Name = 'DailyToplineReport')
        DROP TABLE Marketing.DailyToplineReport
    ...
    select YEAR(a.DateOrdered) YearOrdered,
       ...
       -- fo.CustType,		--PR 9/12 -- This variable is causing confusion. So, removing after discussing with Amit
        ...
        vac.MD_CampaignName,
        vac.IPR_Channel
    into Marketing.DailyToplineReport	
