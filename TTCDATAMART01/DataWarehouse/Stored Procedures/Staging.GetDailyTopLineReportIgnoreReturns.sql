...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [Staging].[GetDailyTopLineReportIgnoreReturns] 
AS
	-- Preethi Ramanujam    2/6/2012 - Get daily top line report
begin

	if object_id('Staging.DailyToplineReportIgnoreReturns') is not null drop table Staging.DailyToplineReportIgnoreReturns
  ...
  --		Mapping.CustomerOverLay co on co.CustomerID = a.CustomerID  left join
    ...
    order by 1,2	
  	...
  	if object_id('Marketing.DailyToplineReportIgnoreReturns') is not null drop table Marketing.DailyToplineReportIgnoreReturns
    alter schema Marketing transfer Staging.DailyToplineReportIgnoreReturns
