...
create PROCEDURE [Staging].[LoadRFMHistory_BKP_DropDRTV]
	@parmDate DATETIME
AS
    declare 
    	@effDate DATETIME,
...
begin
	...
	BEGIN
        ...
        Print 'ProcessingDate - ' + convert(varchar(10), @parmDate, 101)
    	
        EXECUTE Staging.LoadRFMData @parmDate, 'RFMHistory'
        
        -- Need to Drop DRTV Folks
        IF @parmDate > '3/1/2012'
		begin
			delete a
			from Staging.RFMDataRH (nolock) a join
				Marketing.DRTV_BuffetCustomers b on a.CustomerID = b.customerid
    	end
    	
        INSERT INTO Archive.RFMHistory (CustomerID, RFM, EffectiveDate, a12mf, 
                        cnt_of_orderID, ProcessingDate)
            SELECT CustomerID, Concatenated, @effDate, a12mf, CountOfOrderID, @parmDate
            FROM Staging.RFMDataRH (nolock)
    ...
    GROUP BY  rh.effectivedate, rh.processingdate, ri.NewSeg, rn.Name, rh.a12mf
