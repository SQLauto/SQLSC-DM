...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[SP_Process_UK_PM_Mailfile]          
as          
BEGIN          
...
declare @table_nm varchar(200), @subjectFlg bit            
 ...
 update a          
 set  Customerid = cast (replace(Customerid,'CustomerID: ','') as int) , adcode = cast(replace(adcode,'Priority Code: ','') as int)          
 ...
 where adcode in (99993,99994,99995,99996,99997,99998,99999,11844,12205,12345)            
  ...
  where countrycd='UK' and map.PMUpdateflag=0 )) PM             
  on pm.adcode=map.adcode            
  where map.PMUpdateflag=0 and map.CountryCd = 'UK'             
...
if @table_nm is not null            
  ...
  insert into [Archive].[ToMH_PM_MailFile] (CustomerID,adcode)   select distinct a.Customerid, a.adcode   from [Archive].[ToMH_PM_MailFile_test] A               
