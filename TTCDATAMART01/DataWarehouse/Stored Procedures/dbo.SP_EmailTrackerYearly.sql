...
CREATE Proc [dbo].[SP_EmailTrackerYearly] @Year int, @Month int = 0 
AS
					...
					select 	isnull(EH.adcode,@adcode)adcode,isnull(EH.comboid,'CCN2M') comboid,
							case when FlagHoldOut = 1 then 0 
								 when EH.CustomerID is null and DM.CustomerID is not null then 2
								  ...
								  EH.CustomerID EmailHistCutomerID,EH.FlagHoldOut,EH.PreferredCategory
...
/**************************************************      #EmailTracker      ***********************************************************/
			...
			select 
					A.AdCode,A.AdcodeName,A.CatalogCode,A.CatalogName,A.ComboID,Count( Distinct T.EmailHistCutomerID) as TotalEmailed,A.EmailSentDate as EHistStartDate,
