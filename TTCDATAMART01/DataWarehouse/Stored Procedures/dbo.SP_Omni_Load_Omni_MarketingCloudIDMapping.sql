...
select distinct  case when isnull(cust.JSMERGEDROOT,'') <> '' then cust.JSMERGEDROOT                          
 ...
 from MagentoImports..Email_Customer_Information ECI                
 left join DataWarehouse.Archive.Omni_MarketingCloudIDMapping OMC  
 on ECI.web_user_id = OMC.userID         
 left join DAXImports..CustomerExport Cust        
 on ECI.dax_customer_id = Cust.Customerid         
 where isnull(dax_customer_id,'')<>'' and web_user_id <> ''


delete  from   DataWarehouse.mapping.Customerid_Userid_MarketingCloudID
