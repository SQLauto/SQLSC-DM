...
CREATE Proc [dbo].[SP_SubjectConvertalog]     @RunSecondPortion bit = 0   
as        
Begin        
...
Begin        
				...
				*/        
				...
				If exists (select 1 from  Datawarehouse.Staging.SubAdcodeConvertalog where CustGroup = 'Test')        
				  ...
				  select   isnull(ComboID,'NoComboID')ComboID , isnull(HVLVGroup,'HV')HVLVGroup,COUNT(*) CNT,cast(0 as int) Processed          
				  Into #Split        
				  from    Datawarehouse.Staging.SubConvertalog        
				  group by   isnull(ComboID,'NoComboID'), isnull(HVLVGroup,'HV')        
        
        
				  Declare @ComboId varchar(50), @HVLVGroup varchar(10),@Cnt varchar(10)--, @SQL nvarchar(2000)        
				 /*Update Customer group with Test if it exists by splitting customers by comboid and HVLV*/        
					while exists (select top 1 ComboId from #Split where Processed = 0)        
					begin        
        
					select top 1 @ComboId  = ComboId, @HVLVGroup = HVLVGroup ,@Cnt = cast(CNT/2 as int)        
					...
					set CustGroup = ''TEST''        
					where CustomerID in         
					( select top ' + @Cnt + ' customerid from Datawarehouse.Staging.SubConvertalog where isnull(ComboId,''NoComboID'')  = ''' +@ComboId +''' and  isnull(HVLVGroup,''HV'') = ''' + @HVLVGroup  + ''' ) '        
        
					exec (@SQL)        
        
					update  #Split         
					set Processed = 1        
					where ComboId = @ComboId and HVLVGroup = @HVLVGroup        
				...
				*/        
				...
				from Datawarehouse.Staging.SubConvertalog C        
				inner join Mapping.Vw_ConvertalogSubjectCat Subcat         
				on C.SubjectLong2 = Subcat.ConvertalogSubject        
				and C.CustGroup = Subcat.CustGroup        
				inner join Datawarehouse.Staging.SubAdcodeConvertalog Ad         
				on Ad.ConvertalogSubjects = SubCat.ConvertalogSubjectcat        
				and c.HVLVGroup = Ad.HVLVGroup        
