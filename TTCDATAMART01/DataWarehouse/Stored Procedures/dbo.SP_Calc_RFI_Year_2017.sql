...
CREATE proc [dbo].[SP_Calc_RFI_Year_2017] 
as
Begin

--return 0

--insert into [Mapping].[offer_offercodeid_2017] 
...
select top 10 * from [Mapping].[offer_offercodeid_2017]
select top 10 * from  [Mapping].[adcode_offercodeid_2017]

update [Mapping].[adcode_offercodeid_2017]
set OfferCodeID = 502
...
if object_id('staging.EmailTypeConversion2017')  is not null 
drop table staging.EmailTypeConversion2017

select CatalogCode,CatalogName,MA.Adcode,MO.Offer as EmailOffer,MA.OfferCodeID as EmailOfferID 
into  [staging].[EmailTypeConversion2017]
from [Mapping].[adcode_offercodeid_2017] MA 
join [Mapping].[offer_offercodeid_2017] MO 
...
if object_id('Mapping.EmailTypeConversionTable2017')  is not null 
Drop table Datawarehouse.Mapping.EmailTypeConversionTable2017
...
join datawarehouse.mapping.EmailTypeConversionTable2017 b on a.CatalogCode=b.catalogcode
left join (select offer as EmailOffer, OfferCodeID as EmailOfferId from [Mapping].[offer_offercodeid_2017]) c on b.emailofferid = c.EmailOfferID
...
if object_id('Marketing.Email_RFI_Report2017')  is not null 
drop table Datawarehouse.Marketing.Email_RFI_Report2017
 

select * 
into Datawarehouse.Marketing.Email_RFI_Report2017
from Datawarehouse.dbo.EmailReportPrep1


select count(*)Cnts_Email_RFI_Report2017 from  Datawarehouse.Marketing.Email_RFI_Report2017
