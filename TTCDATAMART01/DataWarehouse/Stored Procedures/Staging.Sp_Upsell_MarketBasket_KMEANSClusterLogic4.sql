...
SET ANSI_NULLS ON
GO
CREATE Proc [Staging].[Sp_Upsell_MarketBasket_KMEANSClusterLogic4]    @ProcessLogic int = 0  
as      
Begin      
...
/************* Check # of customers and if <500 drop cluster as those customers will get default ranking later ***********/      
      ...
      select customerid,cluster_ordered as cluster into #CLUSTERS from testsummary.dbo.AV_final_clusters
...
Truncate table Datawarehouse.archive.tgc_upsell_Logic4ListCourseRank      
...
/*****************Load into Final Table************************/      
              ...
              insert into Datawarehouse.archive.tgc_upsell_Logic4ListCourseRank          
...
/* Default course 0*/    
insert into Datawarehouse.archive.tgc_upsell_Logic4ListCourseRank (PrimaryCourseid,SecondaryCourseid,Cluster,FinalRank)    
select 0,courseid,Cluster,rank from      
(select distinct cluster from #CustPurchasedCourse )a , #Bestsellers    
    
      
    
truncate table Datawarehouse.archive.tgc_upsell_Logic4ListCustomerSegment      
insert into Datawarehouse.archive.tgc_upsell_Logic4ListCustomerSegment      
...
End     --If @ProcessLogic = 1   
 ...
 Truncate table Staging.Logic4CustomerList         
 Truncate table Staging.Logic4ListCourseRank         
 ...
 /*********** Load Customer list ***********/        
  ...
  INSERT INTO staging.logic4customerlist         
       (customerid,         
        listid)         
    SELECT F.customerid,  LL.ListID       
     FROM   DataWarehouse.mapping.TGC_Upsell_Logic_List LL        
     left join archive.tgc_upsell_Logic4ListCustomerSegment F        
     on  f.cluster = ll.ListName and LL.logicid = 4      
     where F.customerid > 0        
      
  Declare @MaxCustomerListID SmallInt         
          
  select @MaxCustomerListID = ListID        
  from  ( select top 1 listid,count(*)cnt from  [Staging].[Logic4CustomerList]        
    group by listid         
    order by 2 desc        
     ) b        
  ...
  where LL.logicid = 4 and ListName = '0'         
  ...
  select @DefaultListID '@DefaultListID'      
    ...
    FROM   DataWarehouse.Marketing.CampaignCustomerSignature CR         
     left join staging.logic4customerlist F        
  ...
  /********Load Segment Course view Rank ********/        
   Insert into Staging.Logic4ListCourseRank        
        
   SELECT  listid,         
      c.PrimaryCourseid as Courseid,         
      C.FinalRank,        
      c.SecondaryCourseid as UpsellCourseid      
   FROM archive.tgc_upsell_Logic4ListCourseRank C       
   ...
   SELECT  @DefaultListID,         
      ...
      C.FinalRank,        
      c.SecondaryCourseid as UpsellCourseid      
   FROM archive.tgc_upsell_Logic4ListCourseRank C --testsummary.dbo.US_MBA_RecommendList C        
   JOIN mapping.tgc_upsell_logic_list LL         
   ON C.Cluster = LL.listname       
   AND LL.logicid = 4         
   Where FinalRank < 501      
