...
CREATE PROC [Staging].[SP_Load_TGCPlus_Transaction]
As
Begin
...
into #billing
from DataWarehouse.Archive.TGCPlus_UserBilling b


--PaymentAuth

select p.pa_user_id as Customerid,p.pas_created_at as Eventdate,'PaymentAuthStatus' as Event ,p.pas_id id, pas_state as Eventstatus, p.pas_plan_id AS plan_id,SP.billing_cycle_period_type AS SubType, p.pas_payment_handler AS payment_handler
...
select p.pa_user_id as Customerid,p.uso_applied_at as Eventdate,'PaymentAuthOffer' as Event ,p.uso_id id, ISNULL(uso_offer_code_applied,'No Offer Description') as Eventstatus, CAST(NULL AS INT) AS plan_id,CAST(null AS VARCHAR(255)) AS SubType, CAST(null AS VARCHAR(255))AS payment_handler
...
IF OBJECT_ID('Archive.TGCPlus_Transaction') IS NOT NULL
DROP TABLE Archive.TGCPlus_Transaction
...
INTO Archive.TGCPlus_Transaction
...
LEFT JOIN DataWarehouse.Archive.TGCPLus_Userbilling UB
...
IF OBJECT_ID('Archive.TGCPlus_CustomerStatus') IS NOT NULL
DROP TABLE Archive.TGCPlus_CustomerStatus
...
WHEN Eventstatus IN ('Completed') AND LTDPayment = 0 AND (F.Eventdate >= Max_service_period_to  OR CAST(GETDATE() AS date)> F.Max_service_period_to) THEN 'Billing Issue'
WHEN Eventstatus IN ('Completed') AND LTDPayment = 0 AND F.Max_service_period_to IS null THEN 'Billing Issue'
--WHEN Eventstatus IN ('Pending','Completed') AND LTDPayment = 0 AND CAST(GETDATE() AS date)> F.Max_service_period_to THEN 'Recurring Payment Issue'
...
, CASE WHEN CAST(GETDATE()-1 AS date) BETWEEN CAST(f.Max_service_period_from AS DATE) AND CAST(f.Max_service_period_to AS DATE) THEN 1 ELSE 0 END AS ActiveSubscriber 
,GETDATE() AS DMLastupdated
--INTO ##TGCPlus_QC
INTO Archive.TGCPlus_CustomerStatus
FROM Archive.TGCPlus_Transaction F
JOIN (SELECT Customerid,SUM(CAST(Payment AS DECIMAL(5,2))) AS LTDPayment, MAX(Eventrank)Eventrank, SUM( CASE WHEN Eventstatus = 'Refund' AND payment <0 THEN 1 ELSE 0 END) AS Refunds, SUM(CASE WHEN Eventstatus = 'Payment' AND payment >0 THEN 1 ELSE 0 END) Payments
FROM Archive.TGCPlus_Transaction GROUP BY Customerid ) F1
