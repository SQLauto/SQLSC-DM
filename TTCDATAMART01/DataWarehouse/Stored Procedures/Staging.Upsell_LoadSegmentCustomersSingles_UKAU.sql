...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [Staging].[Upsell_LoadSegmentCustomersSingles_UKAU]
AS
BEGIN
	set nocount on
    
    if object_id('Staging.TempUpsell_SegmentCustomersSinglesNEW') is not null truncate table Staging.TempUpsell_SegmentCustomersSinglesNEW

	INSERT INTO Staging.TempUpsell_SegmentCustomersSinglesNEW       	
    SELECT DISTINCT 
    	DPOI.CustomerID, 
        ...
        END As SegmentGroup,
        case	
			WHEN DPO.BillingCountryCode in ('AU','GB') then DPO.BillingCountryCode 
			else ''  
		end as Country		
--	INTO Staging.TempUpsell_SegmentCustomersSinglesNEW       
    ...
    INNER JOIN Marketing.DMPurchaseOrders DPO (nolock) ON DPOI.OrderID = DPO.OrderID 
    INNER JOIN Marketing.DMCustomerStatic DMC (nolock) ON DPO.CustomerID = DMC.CustomerID
    left join Staging.TempUpsell_SegmentCustomersSinglesNEW t (nolock) on t.CustomerID = dmc.CustomerID
    WHERE 	
    ...
    UPDATE Staging.TempUpsell_SegmentCustomersSinglesNEW
	...
	DELETE FROM Staging.TempUpsell_SegmentCustomersSinglesNEW
    where SegmentGroup in (' AUM', 'XAUM', ' AUF')
    
    update Staging.TempUpsell_SegmentCustomersSinglesNEW
    SET SegmentGroup =  replace(Country,' ','') + replace(SegmentGroup,' ','')
