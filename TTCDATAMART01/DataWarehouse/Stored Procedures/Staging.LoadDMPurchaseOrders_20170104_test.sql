...
Create PROCEDURE [Staging].[LoadDMPurchaseOrders_20170104_test]
as
begin
    set nocount on
    

	IF EXISTS (SELECT name FROM sysindexes WHERE name = 'IX_TempDMPurchaseOrders_OrderID')
	Drop INDEX IX_TempDMPurchaseOrders_OrderID on Staging.TempDMPurchaseOrders 

	IF EXISTS (SELECT name FROM sysindexes WHERE name = 'IX_TempDMPurchaseOrders_Temp1')
	Drop INDEX IX_TempDMPurchaseOrders_Temp1 on Staging.TempDMPurchaseOrders 
    ...
    left join (
		...
		IF NOT EXISTS (SELECT name FROM sysindexes WHERE name = 'IX_TempDMPurchaseOrders_OrderID')
		CREATE Unique NONCLUSTERED  INDEX IX_TempDMPurchaseOrders_OrderID on Staging.TempDMPurchaseOrders (OrderID )

		IF NOT EXISTS (SELECT name FROM sysindexes WHERE name = 'IX_TempDMPurchaseOrders_Temp1')
		CREATE NONCLUSTERED INDEX IX_TempDMPurchaseOrders_Temp1 on Staging.TempDMPurchaseOrders  (CustomerID,DateOrdered, OrderID ,sequenceNum)
...
/* Include Prior purchase information 	-- Preethi Ramanujam	1/27/2009*/
  ...
  --      	WHEN SequenceNum = 1 then 'New' 
    ...
    (
        ...
        where 
            left(poi.StockItemID, 2) in ('DV', 'DA')
		except            
        select distinct poi.OrderID
        from Staging.ValidPurchaseOrderItems poi (nolock)
        where 
            left(poi.StockItemID, 2) in ('PD', 'PC', 'PV', 'PA', 'PM', 'LA', 'LC', 'LD', 'LV')
    )
    update po
