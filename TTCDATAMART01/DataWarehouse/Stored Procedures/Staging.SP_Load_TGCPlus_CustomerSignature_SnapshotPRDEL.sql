...
CREATE Proc [Staging].[SP_Load_TGCPlus_CustomerSignature_SnapshotPRDEL] @SnapshotDate Date = null
as
...
exec [Staging].[SP_Load_TGCPlus_Transaction_SnapshotPRDEL] @Date = @SnapshotDate
...
/*Check users who have paid flag =1 and last paid dates is null*/
		Select * from DataWarehouse.Archive.TGCPlus_UserBilling20170802
...
from DataWarehouse.#TGCPlus_CustomerDaily c
join Datawarehouse.Archive.TGCPlus_customerStatus_Snapshot s on c.CustomerID = s.Customerid
where c.TransactionType = 'cancelled' and S.ActiveSubscriber = 1 and S.Status in ('Initial Payment','Recurring Payment')
		

/* Truncate and load TGCPlus_CustomerSignature_SnapshotPRDEL*/

 Delete from  Datawarehouse.Marketing.TGCPlus_CustomerSignature_SnapshotPRDEL
 where AsofDate = @SnapshotDate

		Insert into Datawarehouse.Marketing.TGCPlus_CustomerSignature_SnapshotPRDEL
		select 
				AsofDate,
				a.CustomerID,
				a.uuid,
...
/* Country update to include Country names from datawarehouse.Mapping.TGCPLusCountry vikram 9/20/2016*/
	 ...
	 from Datawarehouse.Marketing.TGCPlus_CustomerSignature_SnapshotPRDEL c
