...
CREATE Proc [Staging].[SP_TGCPLUS_Churn_Working]
as
...
Ignore Duplicates created by Streaming, seqnum = 1

DaybyDay_Consumption will only have free minutes consumed.
ex: if cust has 7 free days then we only do a running total of streamed mins till day 7,and Day7Consumption - daybyday30 will remain same.

StreamedMins_7days,StreamedMins_14days,StreamedMins_30days are not restricted by Freedays.  

Sailthru Email engagements are also not restricted by Freedays.
...
and b.TSTAMP between Cast(a.DS_Service_period_from as date) and Cast(a.DS_Service_period_to as date) --20180405 Changes to include the streamed date
...
from staging.TGCPlus_Consumption_FreeTrial 
 ...
 ----------------------------------------------------------------------------#FreeDays-----------------------------------------------------------------------------------------------------
  ...
  IF OBJECT_ID('staging.TGCPlus_freedays') IS NOT NULL          
    DROP TABLE staging.TGCPlus_freedays    
...
uuid, 
case
	when min(browser) = 'Amazon Silk' then 'Chrome'
	when min(browser) = 'Android Browser' then 'Chrome'
	when min(browser) = 'Android WebView' then 'Chrome'
	when min(browser) = 'Chrome' then 'Chrome'
	when min(browser) = 'Firefox' then 'Firefox'
	when min(browser) = 'Mozilla' then 'Firefox'
	when min(browser) = 'Mozilla Compatible Agent' then 'Firefox'
	when min(browser) = 'IE' then 'IE'
	when min(browser) = 'Internet Explorer' then 'IE'
	when min(browser) = 'Safari' then 'Safari'
	when min(browser) = 'Safari (in-app)' then 'Safari'
	when min(browser) = 'Opera' then 'Opera'
	when min(browser) = 'Opera Mini' then 'Opera'
	else 'Other' end as BrowserSignUp, 
min(DeviceCategory) as FirstDeviceCategory
into #GAInitial
from
(
SELECT mt.uuid, mt.browser, mt.DeviceCategory, mt.Date
    
  FROM Marketing.TGCPLus_GA_Metrics_user (nolock) mt
	inner join
	(
	select uuid, min(Date) MinDate from Marketing.TGCPlus_GA_Metrics_user (nolock) 
	group by uuid
	) t on mt.uuid = t.uuid and mt.Date = t.MinDate 
group by mt.uuid, browser, devicecategory, mt.Date
) as Agg
group by uuid 
...
from
	...
	from 
		   (
					...
					sum(case when Days <= isnull(f.freedays,30) then  FinalStreamedMins else null end ) FSM,
					sum(case when Days <= isnull(f.freedays,30) then LectureRunMins else null end ) LRM,
					sum(case when Days <= isnull(f.freedays,30) then FlagCompletedLecture else null end ) FCL
					from 
					(
					...
					Left join staging.TGCPlus_freedays F on F.CustomerID = c1.CustomerID
...
left join staging.TGCPlus_freedays F on F.CustomerID = a.CustomerID
...
SELECT CustomerID, 
Min(Optout_time) Optout_time,sum([1])Day1,sum([2])Day2,sum([3])Day3,sum([4])Day4,sum([5])Day5,sum([6])Day6,sum([7])Day7,sum([8])Day8,sum([9])Day9,sum([10])Day10,
...
from archive.omni_tgcplus_mcid_uuid_mapping (nolock) a
	...
	left join staging.TGCPlus_freedays f on f.CustomerID = b.CustomerID
	left join
		(
...
(
	...
	b.PrimaryWebCategory as FirstCategory, b.LectureTitle as FirstLecture, b.episodenumber as FirstLectureNumber, b.genre as FirstGenre, b.FilmType as FirstFilmType,row_number() over (partition by a.CustomerID order by Courseid,episodenumber) as Rnk
	from Datawarehouse.archive.vw_tgcplus_customersignature (nolock)  a
			LEFT JOIN staging.TGCPlus_freedays f on f.CustomerID = a.CustomerID
			LEFT JOIN Archive.Vw_TGCplus_VideoEvents_Smry (nolock) b on a.CustomerID = b.id 
	where SeqNum = 1 
...
(
		...
		from 
		(		select a.Customerid,Days,Sum( case when Days <= isnull(f.freedays,30) then Streamedmins else null end)Streamedmins
					from ( 
...
from 
(
	select a.Customerid,Days, Count(distinct case when Days <= isnull(f.freedays,30) then  genre else null end )Genres
	from ( 
			SELECT a.CustomerID,a.uuid,D.Date,a.IntlSubDate,Datediff(d,a.IntlSubDate, D.Date) + 1 AS Days
			FROM DataWarehouse.Archive.vw_tgcplus_customersignature (nolock) a
			LEFT JOIN DataWarehouse.Mapping.Date D ON Cast(D.Date AS date) >= Cast(a.IntlSubDate AS date)
...
from 
(
	select a.Customerid,Days, Count(distinct case when Days <= isnull(f.freedays,30) then LectureTitle else null end )Lectures
	from ( 
			SELECT a.CustomerID,a.uuid,D.Date,a.IntlSubDate,Datediff(d,a.IntlSubDate, D.Date) + 1 AS Days
			FROM DataWarehouse.Archive.vw_tgcplus_customersignature (nolock) a
			LEFT JOIN DataWarehouse.Mapping.Date D ON Cast(D.Date AS date) >= Cast(a.IntlSubDate AS date)
...
Begin Tran
 ...
 IF OBJECT_ID('archive.tgcplus_churnmodeldata') IS NOT NULL          
    DROP TABLE archive.tgcplus_churnmodeldata     

select * into Datawarehouse.archive.tgcplus_churnmodeldata
from Datawarehouse.archive.TGCPLUS_Churn_Working
