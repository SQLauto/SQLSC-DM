...
CREATE PROC [Staging].[ExportTableToCSVText]
	@DbName varchar(25),
...
-- Preethi Ramanujam 1/16/2014
	...
	SELECT @sql = 'BCP "DECLARE @colnames VARCHAR(max);SELECT @colnames = COALESCE(@colnames + '','', '''') + column_name from ' + @DBName + '.INFORMATION_SCHEMA.COLUMNS where TABLE_NAME=''' + @TableName + ''' order by ORDINAL_POSITION; ; select @colnames;" 
queryout "' + @Path + '\' + @Header + '"   -c -t'+'","'+' -T -S ttcdatamart01'
	 print @sql
	exec master..xp_cmdshell @sql
