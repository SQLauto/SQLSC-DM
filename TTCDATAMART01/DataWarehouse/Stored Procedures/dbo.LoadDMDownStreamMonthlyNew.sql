...
CREATE PROCEDURE [dbo].[LoadDMDownStreamMonthlyNew]
as
begin
	set nocount on

If OBJECT_ID('Marketing.DMDownStreamMonthlyNew') is not null 
drop table Marketing.DMDownStreamMonthlyNew
	
	Select DMO.Customerid,
	...
	Into Marketing.DMDownStreamMonthlyNew		
    from DataWarehouse.Marketing.DMPurchaseOrders DMO (nolock)
    left join DataWarehouse.Marketing.DMEmailContactDownStream Email (nolock)
    on DMO.CustomerID = Email.customerid
    left join DataWarehouse.Marketing.DMmailContactDownStream Mail (nolock)
    on DMO.CustomerID = Mail.customerid
    group by DMO.customerid    
end
