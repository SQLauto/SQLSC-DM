...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE Proc [Staging].[SP_TGCPlus_ConsumptionFreeMonthChurn_IB]
As
Begin
...
/* Segment initial Streaming information */
  ...
  left join #video10_ib b on a.uuid = b.UUID 
  left join DataWarehouse.Archive.TGCPlus_Film c on b.Vid = c.uuid

  --select IntlStreamedMinBin, count(*) from #video2_ib
  --group by IntlStreamedMinBin
  --order by 1

  --select IntlPlay_DSISBin, count(*) from #video2_ib
...
/* Calculating LTD values for watched Courses*/
  ...
  into #custltd_ib
  from DataWarehouse.Marketing.TGCplus_Consumption_Smry a 
...
--select * from #custltd_ib2
  ...
  into #video3_ib
  from #video2_ib a 
  left join #custltd_ib2 b on a.uuid = b.UUID
...
/* Calculate Downstream 30 days Streaming*/
  ...
  into #DS0_1_ib
  from DataWarehouse.Marketing.TGCPlus_CustomerSignature a
       left join DataWarehouse.Marketing.TGCplus_Consumption_Smry b 
...
If object_id('Marketing.TGCPlus_ConsumptionFreeMonthChurn_ib') is not null
Drop table Marketing.TGCPlus_ConsumptionFreeMonthChurn_ib
  ...
  into Marketing.TGCPlus_ConsumptionFreeMonthChurn_ib
  from #video3_ib a 
  left join #DS0_2_ib b on a.uuid = b.UUID

--select * from Marketing.TGCPlus_ConsumptionFreeMonthChurn

/*Clean up temp tables*/
drop table #video10_ib
drop table #video2_ib
drop table #custltd_ib
drop table #custltd_ib2
drop table #DS0_1_ib
drop table #DS0_2_ib
