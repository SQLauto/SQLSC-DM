...
CREATE proc [dbo].[SP_Calc_RFI_Year_2016] 
as
Begin
...
select top 10 * from  [Mapping].[offer_offercodeid_2016]
select top 10 * from [Mapping].[adcode_offercodeid_2016]
...
if object_id('staging.EmailTypeConversion2016')  is not null 
drop table staging.EmailTypeConversion2016

select CatalogCode,CatalogName,MA.Adcode,MO.Offer as EmailOffer,MA.OfferCodeID as EmailOfferID 
into  [staging].[EmailTypeConversion2016]
from [Mapping].[adcode_offercodeid_2016] MA 
join [Mapping].[offer_offercodeid_2016] MO 
...
--select top 10 * from Datawarehouse.Mapping.EmailTypeConversionTable2016


if object_id('Mapping.EmailTypeConversionTable2016')  is not null 
Drop table Datawarehouse.Mapping.EmailTypeConversionTable2016
...
join datawarehouse.mapping.EmailTypeConversionTable2016 b on a.CatalogCode=b.catalogcode
left join (select offer as EmailOffer, OfferCodeID as EmailOfferId from [Mapping].[offer_offercodeid_2016]) c on b.emailofferid = c.EmailOfferID
...
if object_id('Marketing.Email_RFI_Report2016')  is not null 
drop table Datawarehouse.Marketing.Email_RFI_Report2016
 

select * 
into Datawarehouse.Marketing.Email_RFI_Report2016
from Datawarehouse.dbo.EmailReportPrep1


select count(*)Cnts_Email_RFI_Report2016 from  Datawarehouse.Marketing.Email_RFI_Report2016
