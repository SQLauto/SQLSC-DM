...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [Staging].[CampaignEmail_CompCollct]
...
BEGIN
    ...
    select @TablePrefix = 'Email' + CONVERT(VARCHAR,@DropDate,112) + '_Offers_CYC'
...
--        DROP TABLE Staging.Ecampn02Rank = Staging.TempECampaignCourseRank
...
--    ORDER BY FNL.PreferredCategory,Sum(FNL.SumSales) DESC
    ...
    /* SET Rank = Rank/2.1*/
    /* WHERE CourseID IN (2270, 8598, 5620)*/
    /* and rank > 10*/
...
--    AND Preferredcategory <> SubjectCategory2
    ...
    delete r
    from Staging.TempECampaignCourseRank r
    join Mapping.DMCourse c (nolock) on r.CourseID = c.CourseID
    left join Staging.Ecampn02_CoursesOnSale s (nolock) on r.CourseID = s.CourseID
    where isnull(s.CourseID, '') = '' and r.PreferredCategory <> c.SubjectCategory2 
    ...
    -- After rank
    print 'After running Staging.CampaignEmail_ExecuteSharedLogic'
    
    alter table Staging.TempECampaignCourseRank
    alter column rank float
    
    
	/* Force courses that are in base course list, but are not in same subject category
		down in ranking */
	print 'Force ranks for some courses to top'

	 UPDATE a
	 SET a.Rank = Rank + 5.988
	 from Staging.TempECampaignCourseRank a 
	 join Mapping.DMCourse c (nolock) on a.CourseID = c.CourseID
	 WHERE a.CourseID not IN (3120,3742,3757,8320,9438,5610,2120,2310,7240,780,6121,1220,1527,1823,1235,1557,5642,5665,5932,5511,1564,2241,1600,2100,2332,243,2140,9422,8922,810,730,740,7250,9313,5913,5943,5964,9388,6299,6593,656,6260,6252,6362,687,1852,158,1257,1252,7150,7820,7510,7187) -- 19.95 courses
	 and a.PreferredCategory <> c.SubjectCategory2 
	 --and rank > 10
...
--        Ecampn03Customer01 = TempECampaignEmailableCustomers
...
--        CASE WHEN ccs.CustomerSegmentNew = 'Active' THEN @AdcodeActive
   ...
   /*- Update AdCode for International customers*/
    ...
    FROM Staging.TempECampaignEmailableCustomers EC01 JOIN
...
--        Ecampn03Customer02_Course -> TempECampaignCustomerCourseRank
...
--    ON Staging.Ecampn04Customer_Final (Customerid)
	...
	exec Staging.CampaignEmail_ExecuteSharedLogic 'ReRankCourses'    
...
--    END
    ...
    exec Staging.CampaignEmail_ExecuteSharedLogic 'CreateFinalTables', @AdCodeActive, @StartDate, @TablePrefix = @TablePrefix    
