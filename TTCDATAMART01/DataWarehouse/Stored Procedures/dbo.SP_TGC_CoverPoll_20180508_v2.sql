...
create PROC [dbo].[SP_TGC_CoverPoll_20180508_v2]
as
...
set  @query1 = 'select a.CustomerID,a.FirstName,a.LastName,c.MaxOpenDate,a.Frequency as SingleOrMulti,c.EmailAddress,0 as FlagSelected ,a.LastOrderDate,a.CountryCode,a.PreferredCategory2,a.MediaFormatPreference,a.OrderSourcePreference ,convert(int,0) as CustGroup into ' + @tablenm + ' from DataWarehouse.Marketing.CampaignCustomerSignature a join (select distinct CustomerID from DataWarehouse.Marketing.DMPurchaseOrders where DateOrdered between  DATEADD(DAY, 1, EOMONTH(getdate(), -3)) and DATEADD(DAY, 1, EOMONTH(getdate(), -1)) )b on  a.CustomerID = b.CustomerID join (select * from DataWarehouse.Marketing.Vw_EPC_EmailPull where EmailFrequency = 1 and FlagDormantCustomer = 0) c on a.CustomerID = c.CustomerID where a.PublicLibrary = 0 and a.CountryCode = ''US'' and a.BuyerType = 4'

exec (@query1)

set @query2= 'delete a from ' +  @tablenm + ' a join DataWarehouse..Polling_MasterOptOut b on a.Emailaddress = b.EmailAddress'

exec (@query2)
set @query3 = 'delete a from ' +@tablenm +' a join DataWarehouse..Polling_MasterOptOut b on a.Emailaddress = b.EmailAddress2'

exec (@query3)
	
set @query4 ='delete a from '+@tablenm+' a join DataWarehouse..Polling_MasterOptOut b on a.CustomerID = b.CustomerID'
exec (@query4)
	
set @query5 ='delete a from ' + @tablenm + ' a join DataWarehouse..Polling_MasterSuppression b on a.CustomerID = b.CustomerID'
exec (@query5)
...
set @query6 = 'delete a from '+ @tablenm+' a join (select * from datawarehouse.archive.RD_Polling_History where Business = ''TGC'' and PullDate >= dateadd(day,-40,getdate())) b on a.CustomerID = b.CustomerID'
exec (@query6)

set @query7 ='delete a from '+ @tablenm+ ' a join (select * from datawarehouse.archive.RD_Polling_History where Business = ''TGC'' and PullDate >= dateadd(day,-40,getdate()))b on a.Emailaddress = b.EmailAddress'
exec (@query7)
...
exec (@query9)
 ...
 set @query10= 'delete a from ' + @tablenm + ' a join ' + @temp3 +' b ON A.CustomerID = b.CustomerID and a.MaxOpenDate = b.Maxopendate'

exec (@query10)


set @query12 = 'update a set a.flagselected = 0, a.CustGroup = 0 from '  + @tablenm + ' a'
