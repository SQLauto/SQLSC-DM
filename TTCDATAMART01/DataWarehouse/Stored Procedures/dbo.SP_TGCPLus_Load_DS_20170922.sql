...
CREATE Proc [dbo].[SP_TGCPLus_Load_DS_20170922]
As
...
---Additional Issues
 ...
 --Additional Issues DS Dupes due to 0 days entitlement and change in plans
  ...
  Amount,NetAmount,DSSplits,PaidFlag,BillingDupes,	UBIssue, MinDS,MinDSDate,MaxDS,MaxDSDate,LTDAmount,
  ...
  Case when Coalesce_DSDate = MaxDSDate then 1 else 0 end as CurrentDS,uso_offer_id,uso_offer_code_applied,uso_applied_at,
  ...
  (
		  ...
		  from 
		  (select Customerid,min(DS)MinDS,cast(Min(DS_Service_period_from) as Date) MinDSDate,Max(DS)MaxDS,cast(Max(Coalesce_DSDate) as date)as MaxDSDate, Sum(Amount) As LTDAmount 
		  from Datawarehouse.Staging.TGCplus_DS_Working
