...
select  s.amazon_order_id, 
		...
		cast(Substring(replace(s.purchase_date,'t',' '),1, charindex('+',replace(s.purchase_date,'t',' '))-1) as datetime)purchase_date, 
		cast(Substring(replace(s.payments_date,'t',' '),1, charindex('+',replace(s.payments_date,'t',' '))-1) as datetime)payments_date, 
		cast(Substring(replace(s.shipment_date,'t',' '),1, charindex('+',replace(s.shipment_date,'t',' '))-1) as datetime)shipment_date, 
		cast(Substring(replace(s.reporting_date,'t',' '),1, charindex('+',replace(s.reporting_date,'t',' '))-1) as datetime)reporting_date, 
		s.buyer_email, 
		s.buyer_name, 
		...
		s.carrier, 
		s.tracking_number, 
		cast(Substring(replace(s.estimated_arrival_date,'t',' '),1, charindex('+',replace(s.estimated_arrival_date,'t',' '))-1) as datetime)estimated_arrival_date, 
