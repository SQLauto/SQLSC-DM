...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [Staging].[GetDailyTopLineReportWithAdcode] 
AS
	-- Preethi Ramanujam    2/6/2012 - Get daily top line report
begin

    IF EXISTS (SELECT 8 FROM sysobjects WHERE Name = 'DailyToplineReportWithAdcode')
        DROP TABLE Marketing.DailyToplineReportWithAdcode
    ...
    select YEAR(a.DateOrdered) YearOrdered,
       ...
       -- fo.CustType,		--PR 9/12 -- This variable is causing confusion. So, removing after discussing with Amit
        ...
        vac.MD_CampaignName,
        vac.IPR_Channel
    into Marketing.DailyToplineReportWithAdcode	
  ...
  --		Mapping.CustomerOverLay co on co.CustomerID = a.CustomerID  left join
    ...
    -- where year(a.DateOrdered) >= YEAR(getdate())-1  -- changed based on Joe's request on 1/3/2018
