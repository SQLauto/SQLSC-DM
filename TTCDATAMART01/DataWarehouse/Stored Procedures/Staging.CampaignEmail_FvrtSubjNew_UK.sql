...
CREATE PROC [Staging].[CampaignEmail_FvrtSubjNew_UK]
...
SELECT @FnlTable = 'Ecampaigns.dbo.Email' + CONVERT(VARCHAR,@DropDate,112) + '_' + 'Offers_FvrtSbjctUK'
/* SELECT @FnlTable = 'Email' + CONVERT(VARCHAR,@DropDate,112) + '_' + 'Offers_FSDwnlds'*/
FROM Staging.MktCatalogCodes
WHERE CatalogCode = @EmailCatalogCode

SELECT @FnlTableSNI1 = 'Ecampaigns.dbo.Email' + CONVERT(VARCHAR,@DropDateSNI,112) + '_' + 'Offers_FvrtSbjctUK'
