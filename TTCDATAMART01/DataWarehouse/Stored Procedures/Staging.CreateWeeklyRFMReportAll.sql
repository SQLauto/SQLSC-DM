...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [Staging].[CreateWeeklyRFMReportAll] @Date DATETIME = '1/1/1900'
AS
BEGIN
    --set nocount on
...
-- PR -- 3/22/2013 - Add UK and AU customer count by RFM as per Bryan's Request
/* Update final report with College customer Count for GB*/
PRINT 'Update final report with College customer Count for GB'
UPDATE A
SET A.GB_FinalCount = B.CollegeCustCount
FROM Staging.Weekly_RFM_Report A JOIN
	(select NewSEg,Name,A12mf, count(customerid) CollegeCustCount
	from Staging.TempWeeklyRFMReport
	where CountryCode = 'GB'
...
/* Update final report with HighSchool Customer Count for GB*/
PRINT 'Update final report with HighSchool Customer Count for GB'
UPDATE A
SET A.GB_FinalCount = B.TotalCount
FROM Staging.Weekly_RFM_Report A JOIN
	(SELECT InqType, COUNT(*) TotalCount
	FROM Staging.TempWeeklyRFMReport
	WHERE BuyerType = 3
...
/* Update final report with NoRFM Count for GB*/
PRINT 'Update final report with NoRFM Count for GB'
UPDATE A
SET A.GB_FinalCount = B.TotalCount
FROM Staging.Weekly_RFM_Report A JOIN
	(SELECT InqType, COUNT(*) TotalCount
...
-- PR -- 3/22/2013 - Add UK and AU customer count by RFM as per Bryan's Request
/* Update final report with College customer Count for Australia*/
PRINT 'Update final report with College customer Count for Australia'
UPDATE A
SET A.AU_FinalCount = B.CollegeCustCount
FROM Staging.Weekly_RFM_Report A JOIN
	(select NewSEg,Name,A12mf, count(customerid) CollegeCustCount
	from Staging.TempWeeklyRFMReport
	where CountryCode = 'AU'
...
/* Update final report with HighSchool Customer Count for Australia*/
PRINT 'Update final report with HighSchool Customer Count for Australia'
UPDATE A
SET A.AU_FinalCount = B.TotalCount
FROM Staging.Weekly_RFM_Report A JOIN
	(SELECT InqType, COUNT(*) TotalCount
	FROM Staging.TempWeeklyRFMReport
	WHERE BuyerType = 3
...
/* Update final report with NoRFM Count for Australia*/
PRINT 'Update final report with NoRFM Count for Australia'
UPDATE A
SET A.AU_FinalCount = B.TotalCount
FROM Staging.Weekly_RFM_Report A JOIN
	(SELECT InqType, COUNT(*) TotalCount
