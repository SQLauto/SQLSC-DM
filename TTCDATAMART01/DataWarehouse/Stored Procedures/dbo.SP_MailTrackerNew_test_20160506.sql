...
SET ANSI_NULLS ON
GO
create Proc [dbo].[SP_MailTrackerNew_test_20160506] @CatalogCode int  
as  
Begin  
...
--select * from  DataWarehouse.Archive.MailhistoryCurrentYear
...
/* Calculate TotalCourseSales,TotalCourseParts,TotalCourseUnits from Staging.vwOrderItems*/
		 ...
		 --select CustomerID,OrderID,AdCode,NetOrderAmount,TotalCourseSales,TotalCourseParts,TotalCourseUnits,DateOrdered  
		 --into #DM_CatalogCode   
		 --from #VWOrders_CatalogCode  

		 --select CustomerID,OrderID,AdCode,NetOrderAmount,TotalCourseSales,TotalCourseParts,TotalCourseQuantity as TotalCourseUnits,DateOrdered  
		 --into #DM_CatalogCode   
		 ...
		 into #MH_CatalogCode   
		 from DataWarehouse.Archive.MailhistoryCurrentYear MH  
		 where AdCode in (Select adcode from DataWarehouse.Mapping.vwAdcodesAll where  CatalogCode = @CatalogCode ) 
...
/**************************************************      #MailTracker      ***********************************************************/  
  ...
  */
	   ...
	   select   
		 ...
		 A.YearOfMailSent,A.MonthOfMailSent,A.MD_Year,A.MD_Audience,A.MD_Channel,A.MD_PromotionType ,A.MD_CampaignName,A.MD_PriceType,  
		 A.MD_Country,A.StartDate as CCTblStartDate,A.StopDate,A.SeqNum,A.CustomerSegment,A.MultiOrSingle,A.NewSeg,A.Name,A.A12mf,  
		 A.CustomerSegmentFnl,A.CustomerSegment2,A.RFMCells,t.FlagMailed,count(t.DMCustomerID)CustCount, count(T.TotalOrders) TotalOrders,  
