...
CREATE proc [dbo].[Sp_EmailPull_CYC]      
as      
Begin      
   ...
   declare @adcode varchar(10), @PrimaryAdcode varchar(10),@Cnt varchar(15),@comboid varchar(20),@preferredcategory varchar(20)      
     ...
     while exists (select top 1 * from #splitter where processed=0 order by adcode)      
      ...
      exec ('update staging.EmailPull       
          set adcode = '+ @adcode +      
        ' where adcode = '+ @PrimaryAdcode +' and customerID in (select top ' + @Cnt + '  customerID   from staging.EmailPull where adcode = '+ @PrimaryAdcode + ' and comboid= '''+ @comboid + ''' and  preferredcategory  = ''' + @preferredcategory + ''' or
der by NEWID() )')      
