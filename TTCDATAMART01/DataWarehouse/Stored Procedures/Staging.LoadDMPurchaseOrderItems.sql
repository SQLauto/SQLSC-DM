...
CREATE PROCEDURE [Staging].[LoadDMPurchaseOrderItems]    
 ...
 INSERT INTO Staging.TempDMPurchaseOrderItems    
  ...
  FlagPaymentRejected,    
    ...
    WHERE    
        ...
        (oI.StockItemID LIKE '[P][TM]%' or Oi.StockItemID LIKE '[D][T]%' or OI.StockItemID LIKE '[PL][CD]%' or OI.StockItemID LIKE '[PLD][AV]%') -- PR 5/30/2013 -- changed this to remove 'Streaming items from DMPurchaseorderItems' -- Original -- '[PLSD][AV]%')    
																												-- PR 9/7/2016 added Digital Transcript to DMPurchaseOrderItems ('DT')
        and I.ItemCategoryID IN ('Course', 'Bundle')    
    ...
    CASE     
      ...
      WHEN SUBSTRING(StockItemID, 1, 2) IN ('PT','DT') THEN 'P'    -- PR 9/7/2016 added Digital Transcript to DMPurchaseOrderItems ('DT') 
    END    
