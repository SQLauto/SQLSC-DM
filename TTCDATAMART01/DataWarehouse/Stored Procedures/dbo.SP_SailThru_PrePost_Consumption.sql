...
CREATE Proc [dbo].[SP_SailThru_PrePost_Consumption] @Date Date = null  
as  
Begin  
...
where send_time >= @Date  
 ...
 --where Tstamp>= Dateadd(d,-3,@Date)  
  ...
  Into #Final  
  From #Blast B  
  Join Datawarehouse.Marketing.TGCplus_VideoEvents_Smry VS  
  on B.UUID = VS.UUID  
...
alter table #Final add IntlSubType Varchar(255),SubType Varchar(255),CustStatusFlag float, PaidFlag Int


	update F 
	set F.IntlSubType  = tcc.IntlSubType,
		F.SubType = tcc.SubType,
		F.CustStatusFlag = tcc.CustStatusFlag , 
		F.PaidFlag = tcc.PaidFlag 
	From #Final F
	left join DataWarehouse.Marketing.TGCPlus_CustomerSignature tcc
	on f.UUID = tcc.uuid
	--and Datepart(WEEK,F.send_date) = Datepart(WEEK,tcc.AsofDate)
/*
--Historical Update
update F 
set F.IntlSubType  = tcc.IntlSubType,
...
From #Final F
left join #TGCPlus_CustomerSignature_Wkly tcc
on f.UUID = tcc.uuid
and Datepart(WEEK,F.send_date) = Datepart(WEEK,tcc.AsofDate)
...
*/
 ...
 Begin   
  Delete S from Archive.TGCPlus_SailThru_blast_Streaming S  
  join #Final F  
  on F.uuid = S.uuid  
  ...
  Insert into Archive.TGCPlus_SailThru_blast_Streaming   
  ...
  IntlSubType,SubType,CustStatusFlag,PaidFlag)  
