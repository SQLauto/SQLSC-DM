...
CREATE proc [dbo].[Sp_EPC_EmailPull_TEST]                      
as                      
Begin                      
...
where EmailID = @EmailID and DLRFlag = 0 and segmentGroup = 'Active match' and countrycode= 'US'                   
...
where EmailID = @EmailID and DLRFlag = 0 and segmentGroup = 'RAM Control' and countrycode= 'US'                   
    
    
Declare @RAMTestAdcode int = 0                 
select @RAMTestAdcode = isnull(Adcode,0) from   datawarehouse.Mapping.Email_adcode                      
where EmailID = @EmailID and DLRFlag = 0 and segmentGroup = 'RAM TEST' and countrycode= 'US'       
...
set Ram.FlagRemoved=1,     
 DateRemoved = getdate(),    
 RAM.AdcodeRemoved= E.Adcode    
from datawarehouse.staging.EPC_EmailPull E    
inner join DataWarehouse.Archive.RAM_CustomerCohort_Emailtest RAM    
...
from datawarehouse.staging.EPC_EmailPull E     
inner join (    
select customerid,custgroup,Comboidprior from DataWarehouse.Archive.RAM_CustomerCohort_Emailtest    
where FlagRemoved=0    
union    
select MRAM.customerid,MRAM.custgroup,Comboidprior from DataWarehouse.Mapping.RAM_CustomerCohort MRAM    
left join DataWarehouse.Archive.RAM_CustomerCohort_Emailtest RAM    
...
/* update Email table with RAMControl and RAMTest Adcodes*/    
 ...
 insert into DataWarehouse.Archive.RAM_CustomerCohort_Emailtest    
