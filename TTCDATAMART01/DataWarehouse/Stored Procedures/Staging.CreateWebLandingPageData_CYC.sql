...
CREATE PROCedure [Staging].[CreateWebLandingPageData_CYC]
...
/*- Proc Name: 	[Staging].[CreateWebLandingPageData] */
/*- Purpose:	To generate Web landing page backend table. */
/*- Updates:*/
/*- Name				Date		Comments*/
/*- Preethi Ramanujam 	8/27/2014	New*/
...
/*- Declare variables*/
    ...
    SELECT 
    	@WebTable = 'lstmgr.dbo.WebLP_' + @Country + '_' + @StartDateChar + '_' + @Name + '_CourseRank',
        @WebTable2 = 'WebLP_' + @Country + '_' + @StartDateChar + '_' + @Name + '_CourseRank'
	...
	and ii.courseid in (Select distinct CourseID
					from mapping.WebLP_InputCourse)
	...
	/* Create Orders table by Country */
	 if object_id('Staging.WebLP_Orders') is not null 
       drop table Staging.WebLP_Orders
 
	if @Country in ('AU','GB')
		Begin
			select *
			into Staging.WebLP_Orders
    ...
    if object_id('Staging.TempWebLPCourseRank') is not null drop table Staging.TempWebLPCourseRank
    if object_id('Staging.TempWebLPBundleCourse') is not null drop table Staging.TempWebLPBundleCourse   
    if object_id('Staging.TempWebLPBundleCourseByCategory') is not null drop table Staging.TempWebLPBundleCourseByCategory
    if object_id('Staging.TempWebLP') is not null drop table Staging.TempWebLP
    ...
    FROM	(SELECT sum(OI.SalesPrice) AS SumSales,
        ...
        FROM 
            Marketing.CampaignCustomerSignature CSM JOIN
                   Staging.WebLP_Orders O  ON CSM.CUSTOMERID = O.CUSTOMERID JOIN
                   Marketing.DMPurchaseOrderItems OI ON O.ORDERID = OI.ORDERID JOIN
            Staging.InvItem II ON OI.StockItemID = II.StockItemID 
            JOIN Staging.MktPricingMatrix MPM ON MPM.UserStockItemID = II.StockItemID
        WHERE O.SequenceNum > 1
    ...
    INTO Staging.TempWebLPBundleCourse
    ...
    FROM Staging.TempWebLPBundleCourse (nolock)
    ...
    begin
        SELECT Sum(FNL.SumSales) AS SumSales,
            ...
            FNL.CourseID,
            FNL.PreferredCategory
        INTO Staging.TempWebLPBundleCourseByCategory
        FROM	(SELECT sum(OI.SalesPrice) AS SumSales,
					COUNT(distinct O.OrderID) as Orders,
                    BC.CourseID,
                    'GEN' AS PreferredCategory
            FROM  Staging.WebLP_Orders O JOIN
                ...
                FROM Staging.TempWebLPBundleCourse) BC on OI.CourseID = BC.CourseID
            AND O.SequenceNum > 1
        ...
        INSERT INTO Staging.TempWebLPCourseRank
        (
        	SumSales,
            CourseID,
            PreferredCategory,
        ...
        SELECT 
     	AVG(B.SumSales) SumSales,
            ...
            0 as CourseParts,
            1
        FROM Staging.TempWebLPBundleCourse A (nolock)
        JOIN Staging.TempWebLPBundleCourseByCategory B (nolock) ON A.CourseID = B.CourseID
        ...
        (
            ...
            from Staging.TempWebLPCourseRank t (nolock),
				(select distinct t2.PreferredCategory 
				from Staging.TempWebLPCourseRank t2 (nolock) 
        ...
        from Staging.TempWebLPCourseRank t
        join Staging.TempWebLPCourseRank t2 (nolock) on t.CourseID = t2.CourseID
        where
            t.FlagPurchased = 0
	...
	if @LogicName <> 'PreRank'
		begin
			...
			FROM Staging.TempWebLPCourseRank ECR JOIN
	...
	FROM Staging.TempWebLPCourseRank a,
		Mapping.DMCourse b
	...
	--UPDATE Staging.TempWebLPCourseRank
	--SET Rank = rank/12.6
	...
	--update Mapping.WebLP_PrefID
	--set Prefid = PrefID + 1
	
	--select @PrefID = PrefID from Mapping.WebLP_PrefID
	
	set @PrefID = @LPID  -- gives control to Sri
	
	if @LPType = 'Web' 
		Begin
			-- delete ranking of other preferredcategories
			delete From Staging.TempWebLPCourseRank
			where PreferredCategory <> 'GEN'
			
			-- Update GEN to WLP
			update Staging.TempWebLPCourseRank
			set PreferredCategory = REPLACE(PreferredCategory,'GEN','WLP')
		end

	update Staging.TempWebLPCourseRank
	set preferredcategory = preferredcategory + CONVERT(varchar,@PrefID)

	update Staging.TempWebLPCourseRank
	set preferredcategory = replace(preferredcategory, ' ','')

	select * from Staging.TempWebLPCourseRank
	order by RANK
	...
	into Staging.TempWebLP		
	from  Staging.TempWebLPCourseRank
    ...
    set @SQLStatement = 'select * into ' + @WebTable + ' from Staging.TempWebLP (nolock) order by rank'
    print (@SQLStatement)
    exec sp_executesql @SQLStatement
     
	/* if @LoadFinal flag is set to 'Y' then load the data to final table */
	if @LoadFinal = 'Y'
		begin
			--insert into reports..mktwebcategorycourses  -- Sri does not want this
			...
			--from Staging.TempWebLP
			
   
	/* Get QC and Information for ticket update */
	
	
	select * from Staging.TempWebLP
		...
		--set @Qry = '
		--select CategoryID, COUNT(*) as CourseCount from
		--reports..mktwebcategorycourses
		--where CategoryID like ''%' + CONVERT(varchar,@PrefID) + '%''
		--group by CategoryID
		--order by 1'
		...
		--set @Qry = '
		--select preferredcategory as categoryid, COUNT(*) as coursecount
		--from Staging.TempWebLP
		...
		print 'Final table reports..mktwebcategorycourses was not loaded with data'
		print 'Update table ' + @WebTable + ' and proceed'
