...
CREATE Proc [dbo].[SP_NCJFY_MailSummary] @full_ind bit=0
as
Begin
...
select distinct YEAR(a.JFYMailDate) YearOfMailing, 
	MONTH(a.JFYMailDate) MonthOfMailing,
	cast(a.JFYMailDate as date) JFYMailDate,
...
into #temp	
from DataWarehouse.Archive.MailingHistory_NCJFY a 
	left join DataWarehouse.Mapping.vwAdcodesAll c on a.adcode = c.adcode
	where a.JFYMailDate>=@date
...
if @full_ind=1										
	begin
		...
		IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Marketing].[MailTracker_NCJFY]') AND type in (N'U'))
		drop table DataWarehouse.[Marketing].[MailTracker_NCJFY]


	select YearOfMailing, MonthOfMailing, JFYMailDate, 
		AdCode, AdcodeName,
	...
	into Datawarehouse.Marketing.MailTracker_NCJFY	
	from #temp
	group by YearOfMailing, MonthOfMailing, JFYMailDate, 
		AdCode, AdcodeName,
		CatalogCode, CatalogName,
		MD_Year, MD_Country, MD_Audience,
		MD_ChannelID, MD_ChannelName,
...
Else
	...
	from Datawarehouse.Marketing.MailTracker_NCJFY a
	inner join #temp2 b
	...
	insert into Datawarehouse.Marketing.MailTracker_NCJFY
