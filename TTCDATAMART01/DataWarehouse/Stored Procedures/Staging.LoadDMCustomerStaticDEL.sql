...
SET ANSI_NULLS ON
GO
create PROCEDURE [Staging].[LoadDMCustomerStaticDEL]
AS
BEGIN
	set nocount on
    ...
    UPDATE MS
    SET MS.IntlPurchaseOrderID = MO.OrderID,
    ...
    INNER JOIN Marketing.DMpurchaseOrders MO (nolock) ON MS.CustomerID = MO.CustomerID
    ...
    FROM 	Staging.TempDMCustomerStatic MCO, (
            ...
            FROM Staging.vwOrders WHERE FlagCouponRedm = 1
            GROUP BY CustomerID) T
    WHERE 	MCO.CustomerID = T.CustomerID 
    ...
    FROM Staging.TempDMCustomerStatic MCS 
    INNER JOIN Staging.Customers C ON MCS.CustomerID = C.CustomerID
    
    UPDATE MCS
    SET MCS.FlagFirstEMailPref = 1
    FROM Staging.TempDMCustomerStatic MCS INNER JOIN Staging.AccountPreferences AP
    ...
    Marketing.DMpurchaseOrders O INNER JOIN
    Mapping.DMPromotionType MPT ON O.CatalogCode = MPT.CatalogCode WHERE
    ...
    SET OrganizationID = 
        ...
        END
    FROM Staging.TempDMCustomerStatic MCS 
    JOIN Staging.CUSTOMERS C ON MCS.CUSTOMERID = C.CUSTOMERID
    
	/* Update intial promotion type, gender, numhits, dwelltype*/

    UPDATE MCS
    SET IntlPromotionType = isnull(MO.PromotionType, 0)
    FROM Staging.TempDMCustomerStatic MCS INNER JOIN Marketing.DMPurchaseOrders MO
    ...
    FROM Staging.TempDMCustomerStatic DCS INNER JOIN Mapping.DMRegion DR
    ON DCS.FirstState = DR.State
    
    UPDATE DCS
    SET DCS.Gender = DI.Gender,
        DCS.ReliabilityCode = DI.DwellType
    FROM Staging.TempDMCustomerStatic DCS INNER JOIN Mapping.DMMPINput DI
    ON DCS.CustomerID = DI.Customerid
    ...
    WHERE CustomerID IN (SELECT CustomerID FROM Staging.vwValidCustomers)
...
/*------------------------------------------------------------------------------*/
	...
	*/    
    ...
    FROM Staging.TempDMCustomerStatic DMC INNER JOIN Marketing.DMPurchaseOrders DPO ON DMC.CustomerID = DPO.CustomerID 
    ...
    UPDATE TempDMCustomerStatic SET 
        ...
        SPFormatAV = 'P'
    WHERE CustomerID IN 
    (SELECT DISTINCT DPO.CustomerID from Marketing.DMPurchaseOrders DPO 
        where
            DPO.SequenceNum = 2 and DPO.CustomerID IN 
    (SELECT CustomerID FROM TempDMCustomerStatic where 
        SPFormatMedia = 'X'))
    ...
    FROM Staging.TempDMCustomerStatic DMC JOIN
        Marketing.CampaignCustomerSignature CCS ON DMC.CustomerID = CCS.CustomerID

	if object_id('Marketing.DMCustomerStatic') is not null
    drop table Marketing.DMCustomerStatic
	select * 
