...
------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [Magento].[P_Get_Upsell_Just_For_You]
...
BEGIN
	...
	BEGIN TRY
		...
		WHERE [Schema_Name] = 'Magento'
			AND Table_Name = 'tgc_upsell_just_for_you' 
		...
		WHERE database_id = DB_ID( 'Exports')
			AND OBJECT_ID=OBJECT_ID('magento.tgc_upsell_just_for_you')
		...
		BEGIN
			...
			SELECT CONVERT(VARCHAR(64),List_ID) AS list_id,
				ranked_recommended_course_ids
			FROM Magento.V_pivot_tgc_upsell_just_for_you
			...
			ELSE
			BEGIN
				RAISERROR('Upsell Export: Exports.Magento.tgc_upsell_just_for_you is empty.',16,1)
			END
