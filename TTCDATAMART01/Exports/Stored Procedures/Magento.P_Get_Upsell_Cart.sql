...
------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [Magento].[P_Get_Upsell_Cart]
...
BEGIN
	...
	BEGIN TRY
		...
		WHERE [Schema_Name] = 'Magento'
			AND Table_Name = 'tgc_upsell_cart' 
		...
		WHERE database_id = DB_ID( 'Exports')
			AND OBJECT_ID=OBJECT_ID('magento.tgc_upsell_cart')
		...
		BEGIN
			...
			SELECT CONVERT(VARCHAR(64),List_ID) AS list_id,
				...
				ranked_recommended_course_ids
			FROM Magento.V_pivot_tgc_upsell_cart
			...
			ELSE
			BEGIN
				RAISERROR('Upsell Export: Exports.Magento.tgc_upsell_cart is empty.',16,1)
			END
