...
------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [Magento].[P_Get_Upsell_Product_Detail_Page]
...
BEGIN
	...
	BEGIN TRY
		...
		WHERE [Schema_Name] = 'Magento'
			AND Table_Name = 'tgc_upsell_product_detail_page' 
		...
		WHERE database_id = DB_ID( 'Exports')
			AND OBJECT_ID=OBJECT_ID('magento.tgc_upsell_product_detail_page')
		...
		BEGIN
			...
			SELECT CONVERT(VARCHAR(64),List_ID) AS list_id,
				CONVERT(INT,Course_ID) AS course_id,
				ranked_recommended_course_ids
			FROM Magento.V_pivot_tgc_upsell_product_detail_page
			...
			SELECT 'Expected Records' AS list_id, 
				@Count AS course_id, 
				'' AS ranked_recommended_course_ids
			...
			BEGIN
				...
				SELECT list_id, 
					course_id, 
					ranked_recommended_course_ids	-- this is done to guarantee count is first
				FROM @Output_Table
				ORDER BY CASE 
			...
			ELSE
			BEGIN
				RAISERROR('Upsell Export: Exports.Magento.tgc_upsell_product_detail_page is empty.',16,1)
			END
		...
		BEGIN
			...
			SELECT list_id, course_id, ranked_recommended_course_ids	
			FROM (
